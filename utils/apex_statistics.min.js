...
var Apex = {
	        ...
	        if (res.code) {
	        	wx.request({
				  ...
				  success: function(ret) {
				    ...
				    systemInfo["visitorId"] = openid; // 唯一标识
				...
				success: function(res) {
					if (res.authSetting["scope.userInfo"]) {
						wx.getUserInfo({
							...
							success: function(res) {
                  addressObj.country = res.userInfo.country || '',
                  addressObj.province = res.userInfo.province || '',
                  addressObj.city = res.userInfo.city || ''
							}
						});
					}
				},
  ...
  getFormatTime: function () {
	...
	sendEvent: function(event) {
	  Apex.getUserSetting();
    let dataMap = {};
    for (var k in systemInfos) {
      dataMap[k] = systemInfos[k]
    }
    if (!!event) {
      dataMap['screenWidth'] = wx.getSystemInfoSync().windowWidth || '';
      dataMap['screenHeight'] = wx.getSystemInfoSync().windowHeight || '';
      dataMap['pageX'] = event.changedTouches ? Math.round(event.changedTouches[0].pageX) : '' || '';
      dataMap['pageY'] = event.changedTouches ? Math.round(event.changedTouches[0].pageY) : '' || '';
      dataMap['clientX'] = event.changedTouches ? Math.round(event.changedTouches[0].clientX) : '' || '';
      dataMap['clientY'] = event.changedTouches ? Math.round(event.changedTouches[0].clientY) : '' || '';
      dataMap['offsetLeft'] = event.currentTarget ? Math.round(event.currentTarget.offsetLeft) : '' || '';
      dataMap['offsetTop'] = event.currentTarget ? Math.round(event.currentTarget.offsetTop) : '' || '';
      dataMap['eventName'] = conf.event.register.eventName || 'tap';
      dataMap['eventType'] = 'EVENT';
      dataMap['executionTime'] = Apex.getFormatTime();
      if(!!conf.isProd) {
        console.log(dataMap);
      }
    }
    ...
    systemInfos['third'] = 'apex';
	  systemInfos['address'] = Apex.getUserSetting();
		...
		systemInfos["eventName"] = conf.event.register.eventName || "PV";
		systemInfos["targetPath"] = "";
		systemInfos['userId'] = '';
		systemInfos['unionId'] = '';
		systemInfos['pvId'] = visitIdInfo.req || '';
